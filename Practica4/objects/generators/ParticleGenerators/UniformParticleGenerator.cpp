...
UniformParticleGenerator::UniformParticleGenerator(string n, Particle* model, Vector3 var, float pro,
	ParticleForceRegistry* r, ForceGenerators& _fgs, bool up, Vector3 varP)
	: ParticleGenerator(n, model, var, pro, up, varP, r, _fgs) {
...
list<Particle*> UniformParticleGenerator::generateParticles() {
	...
	if (rndProb(rg) < generation_probability) {
		...
		// Crear partícula
		if (!uniquePoint) newP = model_part->clone(calculatePos(), calculateVel());
		else newP = model_part->clone(model_part->getPos(), calculateVel());
