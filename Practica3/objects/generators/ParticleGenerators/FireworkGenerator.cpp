...
list<Firework*> FireworkGenerator::generateFireworks(Firework* f) {
	...
	for (int i = 0; i < f->getNumberSons(); i++) {
		Firework* newF = nullptr;

		int sons = calculateSons(f->getNumberSons());
		if (uniquePoint) 
			newF = f->clone(f->getPos(), calculateVel(), newLT, sons);
		else 
			newF = f->clone(calculatePos(f->getPos(), f->getRadious()), calculateVel(), newLT, sons);

		newFs.push_back(newF);
		pfr->addRegistry(fgs, newF);
	}
	return newFs;
}
